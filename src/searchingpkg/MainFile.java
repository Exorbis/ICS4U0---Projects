...
public class MainFile {
	...
	public static void main(String[] args) {
		...
		try (BufferedReader br = new BufferedReader(
								 ...
								 new GZIPInputStream(
								 new FileInputStream("C:/Users/Marc/Downloads/SearchingAssignment.txt.gz"))))) {
		...
		*/
		...
		while (true){
			...
			if (breakTest.toLowerCase().trim().equals("stop")){ break; }
			else {
				
				/*
				 * BINARY SEARCH (RADIUS)
				 * This is the gather/inputting of all values where the binarSearch occurs based off the radius parameter as input.
				 */
				int radiusInput = getInt("Enter the radius of the object");
				int searchedRadius = binarySearch(insertedCircles, radiusInput);
				
				if(searchedRadius == -1) {System.out.println("there is no such object with that radius");}
				else {System.out.println("The object with that radius is in position: " + searchedRadius);}
				
				
				/*
				 * LINEAR SEARCH (CIRCLE)
				 * This is the gather/inputting of all values where the linearSearch occurs based off the circle parameter as input.
				 */
				int circleParam1 = getInt("Enter first parameter of circle (radius)");
				String circleParam2 = getString("Enter 3 values seperated by commas to represent x,y,z (vertex)");
				String [] valuesCircle = circleParam2.trim().split(",");
	...
	public static int binarySearch(ArrayList<Circle> array, int radius) {
		...
		while (minimum < maximum){
			...
			if (array.get(middle).radius() < radius){
				minimum = middle + 1;
			} else {
				maximum = middle;
			}
		}
		if (minimum == maximum && array.get(minimum).radius() < radius){return minimum;}
		else {return -1;}
	}
	...
	public static int linearSearch(ArrayList<Circle> array, Circle circle) {
		 ...
		 * Return -1 if you cannot find the item in the array
		 */
		double searchRadius = circle.radius();
		SVector searchVertex = circle.centre();
		
		for (int i = 0; i < array.size()-1; i++){
			if (array.get(i).radius() == searchRadius && array.get(i).centre() == searchVertex){
	...
	public static int linearSearch(ArrayList<Circle> array, Sphere sphere) {
		...
		double searchRadius = sphere.radius();
		SVector searchVertex = sphere.centre();
